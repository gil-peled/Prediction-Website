...
sheet_name = 'Sheet1'
nvars_default = 24
E_max = 8 #refers to 8 etiologies E0 to E8
T_max = 0
...
for E in range(E_max + 1):
    ...
    if E== E_max:
        nvars = nvars_default + 1
    else:
        nvars = nvars_default

    for T in range(T_max + 1):
 
        start_col_b = T * (nvars + 5) + 3 + (column_start -2) #valid because T=1 always
